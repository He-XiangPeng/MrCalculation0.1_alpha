...
def show_linear_line(X_parameters, Y_parameters, Z_parameters):
    ...
    plt.scatter(X_parameters, Z_parameters, marker='p',
                color='k', linewidths=2)  # k = black

    X_parameters.insert(0, [0])
    # print(X_parameters)
    # [[0], [0.02], [0.015], [0.012]]

    # print((regr1.intercept_ * regr.coef_[0] - regr.intercept_ * regr1.coef_[0]) / (regr.coef_[0] - regr1.coef_[0]))
    # print(math.pow((regr1.intercept_ * regr.coef_[0] - regr.intercept_ * regr1.coef_[0]) / (
    # regr.coef_[0] - regr1.coef_[0]) / 1.166 * 100, 1 / 0.871))
    ...
    plt.plot(X_parameters, regr.predict(X_parameters),
             label="$ηsp/c$", color='r', linewidth=1.5)
    plt.plot(X_parameters, regr1.predict(X_parameters),
             label="$lnηr/c$", color='b', linewidth=1.5)
